...
### 批次二：PushPlus 通知渠道与文档迁移

- 新增 PushPlus 通知通道适配器，并补齐请求构造、失败分支与本地 `.env` 回退测试。
- 将 PushPlus token 隔离到 `clawradar/notifiers/pushplus/.env`，同时提供 `.env.example`，避免真实凭证进入仓库。
- 调整交互式启动脚本的通知提示，允许不手填 token、直接读取 PushPlus 目录下的本地配置。
- 删除 README 中的详细版本迭代内容，保留指向 `changes.md` 的说明，把更新过程集中到本文件维护。
