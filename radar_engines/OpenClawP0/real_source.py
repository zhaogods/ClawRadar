...
def _load_mindspider_module():
    ...
    raise RealSourceUnavailableError(
        "MindSpider BroadTopicExtraction 不可用: " + " | ".join(import_errors)
    )
...
def _search_with_tavily(query: str) -> List[Dict[str, Any]]:
    ...
    client = TavilyNewsAgency(api_key=settings.TAVILY_API_KEY)
...
def _search_with_bocha(query: str) -> List[Dict[str, Any]]:
    ...
    client = BochaMultimodalSearch(api_key=settings.BOCHA_WEB_SEARCH_API_KEY)
...
def _search_with_anspire(query: str, limit: int) -> List[Dict[str, Any]]:
    ...
    client = AnspireAISearch(api_key=settings.ANSPIRE_API_KEY)
