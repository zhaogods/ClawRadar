...
选择标准：
1. 查询内容的主题类型（企业品牌、市场竞争、政策分析、公众号发布等）
2. 报告的紧急程度和时效性
3. 分析的深度和广度要求
4. 目标受众和使用场景
5. 输出是否明显面向微信公众号文章、栏目更新或外部传播
...
- 微信公众号热点快评文章模板：适用于热点事件后的快速观点输出、当天可发布的公众号快评、强时效性的热点解读。核心任务是结论前置、快速成稿、突出最值得关注的判断。
- 微信公众号深度解读文章模板：适用于政策变化、行业趋势、技术动态等需要公众号长文解释的场景。核心任务是把复杂变化讲清楚，并提炼对读者最重要的影响判断。
- 微信公众号行业观察周报模板：适用于周更/月更栏目、行业盘点、多事件汇总和周期性公众号内容。核心任务是模块化盘点重点变化，并给出下一阶段关注方向。
- 微信公众号风险提示与辟谣模板：适用于辟谣说明、风险提醒、误读纠偏和负面信息辨析。核心任务是明确已确认信息、划清误读边界，并给出稳妥判断。

当查询中出现以下特征时，应优先考虑微信公众号模板：
- 明确提到“公众号”“微信公众号”“推送”“文章”“长文”“快评”“周报”“辟谣”“风险提示”
- 目标是直接面向外部读者发布，而不是内部正式报告归档
- 需要更强的结论前置、栏目感、可读性和传播性

请尽量返回最精确的模板名称，不要只返回“公众号模板”或“微信模板”这类泛称。
