...
class ZhiHuLogin(AbstractLogin):
    ...
    async def check_login_state(self, login_page_url: str = "") -> bool:
        ...
        if cnt % 20 == 0 and cnt >= 20 and login_page_url:
            utils.logger.info(f"[Zhihu] Active refresh #{cnt // 20} — reloading page...")
            try:
                await self.context_page.goto(login_page_url, wait_until="domcontentloaded", timeout=15000)
                await asyncio.sleep(2)
            except Exception as e:
                utils.logger.info(f"[Zhihu] Refresh goto failed: {e}")
    ...
    async def login_by_qrcode(self):
        ...
        utils.logger.info(f"[ZhiHu.login_by_qrcode] waiting for scan code login, remaining time is {config.QR_LOGIN_WAIT_SECONDS}s")
        try:
