...
- Xvfb — 虚拟帧缓冲，使 Chrome 以非 headless 模式运行以绕过社媒平台反爬检测
  ...
  ```
- zbar-tools + qrencode — QR 码终端高精度渲染（zbarimg 解码 + qrencode 重编码）
  ```bash
...
| 方式 | 说明 |
|------|------|
| 终端渲染 | zbarimg + qrencode 高精度 █ 块字符，手机对终端扫码 |
| HTTP 服务 | 后台自动启动 HTTP 服务，带 7 位 token 鉴权，浏览器打开扫码 |
| PNG 文件 | 保存 `qrcode_login.png`，可 scp 下载后扫码 |

HTTP 服务地址格式：`http://<host>:<port>/<7位token>/qrcode_login.png`

可选在 `.env` 中配置：

```bash
# QR 码 HTTP 鉴权 Token（7位），留空则随机生成
QRCODE_HTTP_TOKEN=
# HTTP 服务端口，默认 8888
QRCODE_HTTP_PORT=8888
# 公网主机地址（NAT/代理场景），默认 127.0.0.1
QRCODE_HTTP_HOST=
```

### 6. PushPlus 通知

配置 `PUSHPLUS_TOKEN` 后，扫码登录时会自动推送通知（含平台名称和扫码地址）到微信。
