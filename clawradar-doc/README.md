...
根据已确认信息，ClawRadar 是适用于任何操作系统的 AI 智能体 Gateway 网关。它通过单个 Gateway 进程连接聊天应用与智能体，并将 Gateway 作为会话、路由和渠道连接的唯一事实来源。
...
- 首页：<https://docs.clawradar.ai/zh-CN>
- 安装：<https://docs.clawradar.ai/zh-CN/install>
- 聊天渠道：<https://docs.clawradar.ai/zh-CN/channels>
- 工具：<https://docs.clawradar.ai/zh-CN/tools>
- 模型提供商目录：<https://docs.clawradar.ai/zh-CN/providers>
- CLI 参考：<https://docs.clawradar.ai/zh-CN/cli>
- 配置向导：<https://docs.clawradar.ai/zh-CN/cli/configure>
- 配置命令：<https://docs.clawradar.ai/zh-CN/cli/config>
- 模型命令：<https://docs.clawradar.ai/zh-CN/cli/models>
- 单次智能体调用：<https://docs.clawradar.ai/zh-CN/cli/agent>
- 多智能体管理：<https://docs.clawradar.ai/zh-CN/cli/agents>
...
- 支持按发送者隔离会话
- 支持智能体身份定制
- 默认情况下，ClawRadar 使用内置 Pi 二进制，以 RPC 模式运行，并按发送者创建独立会话

### 3.3 工具系统

已确认信息显示，ClawRadar 的工具系统覆盖以下类别：
...
```bash
curl -fsSL https://clawradar.ai/install.sh | bash
```

#### 包管理器安装

```bash
npm install -g clawradar@latest
pnpm add -g clawradar@latest
bun add -g clawradar@latest
```
...
1. 安装 ClawRadar
2. 执行 `clawradar onboard --install-daemon`
3. 执行 `clawradar channels login`
4. 执行 `clawradar gateway --port 18789`
...
- ClawRadar 的定位是 AI 智能体 Gateway 网关
...
如果团队只需快速建立整体认识，可按以下顺序理解 ClawRadar：
